...
class GoalA2CSILEagerSync:
    ...
    def __init__(self, h_size, n_actions, model_critic, model_actor, learning_rate_online, weight_mse, weight_sil_mse,
        ...
        #self.optimizer_imitation = tf.train.GradientDescentOptimizer(learning_rate=learning_rate_imitation)
    ...
    def train_imitation(self, s1, s2, s3, y, one_hot_a, advantage, imp_w, max_grad_norm=40):
...
class GoalA2CSILEagerSeparate:
    ...
    def train_imitation(self, s1, s2, s3, y, one_hot_a, advantage, imp_w, max_grad_norm=40):
        ...
        loss_value, grads = self.grad_imitation_critic(self.model_critic, (s1, s2, s3), y, imp_w, self.weight_sil_mse)

        grads, grad_norm = tf.clip_by_global_norm(grads, max_grad_norm)

        self.optimizer_critic.apply_gradients(zip(grads, self.model_critic.trainable_variables), self.global_step)

        loss_value, grads = self.grad_imitation_actor(self.model_actor, (s1, s2, s3), one_hot_a, advantage, imp_w)
