...
class GoalHrlAgent(HrlAgent):
    ...
    def keywithmaxval(d):
        ...
        v = list(d.values())
        k = list(d.keys())
        return k[v.index(max(v, key=lambda x: x[1]))]
    ...
    def update_option(self, sample):
        ...
        if self.target is not None:
            start = self.as_m2s_m[s_m][self.keywithmaxval(self.as_m2s_m[s_m])][0]   # this is not the real starting point!!
        else:
            start = None

        if self.target is not None:
            goal = self.as_m2s_m[s_m_]
        else:
            goal = None
