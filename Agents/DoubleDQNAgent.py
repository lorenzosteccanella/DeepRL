...
class DoubleDQNAgent(AbstractAgent):
    ...
    def __init__(self, action_space, buffer, main_model_nn, target_model_nn, LAMBDA,
    ...
    def act(self, s):
        ...
        if self.state_dimension is None:
            self.state_dimension = s.shape
            self.main_model_nn.Qprediction([np.zeros(self.state_dimension)])
            self.target_model_nn.Qprediction([np.zeros(self.state_dimension)])
    ...
    def _get_tderror(self, batch):
        ...
        for i in range(len(batch)):
            ...
            if done:
                ...
                t[a_index] = r + self.gamma * p_target_[i][np.argmax(p_[i])]
